...
static uint64 leafHash(objectPo o);
static uint64 nodeHash(objectPo o);
static logical leafEquality(objectPo o1, objectPo o2);
static logical nodeEquality(objectPo o1, objectPo o2);

static objectPo findInTree(treePo tree, objectPo key, uint64 hash);
static objectPo findInLeaf(treePo tree, objectPo key, uint64 hash);
static objectPo findInNode(treePo tree, objectPo key, uint64 hash);

static treePo deleteFromTree(treePo tree, objectPo key, uint64 hash);
static treePo deleteFromLeaf(treePo tree, objectPo key, uint64 hash);
static treePo deleteFromNode(treePo tree, objectPo key, uint64 hash);
...
static treePo newNode(int masklen, uint64 mask, treePo l1, treePo l2, treePo r1, treePo r2);
...
static int16 common2WayMaskLen(uint64 h1, uint64 h2) {
  int16 C = 64;
  while (h1 != h2 && C > 0) {
    h1 = h1 >> 2;
    h2 = h2 >> 2;
...
static int16 nth4way(uint64 mask, int16 pos) {
  mask = mask >> (62 - pos);
  return (int16) (mask & 3);
}

static uint64 maskPrefix(uint64 mask, int16 len) {
  if (len == 0)
    return 0;
  else {
    int16 cml = (int16) 62 - len;
    uint64 msb = ((uint64) -1) >> cml;
    uint64 lhsMask = msb << cml;
    return lhsMask & mask;
  }
}
...
static treePo mergeWithLeaf(treePo t1, treePo t2) {
  if (objectHasClass(O_OBJECT(t2), leafClass)) {
    ...
    if (l1->tree.mask == l2->tree.mask) {
      ...
      uint64 cm = maskPrefix(l1->tree.mask, cml);
...
static objectPo findInNode(treePo tree, objectPo key, uint64 hash) {
  ...
  uint64 common = maskPrefix(hash, tree->tree.masklen);
...
static treePo deleteFromNode(treePo tree, objectPo key, uint64 hash) {
  ...
  uint64 mask = n->tree.mask;
  int16 masklen = n->tree.masklen;

  uint64 cm = maskPrefix(hash, masklen);
