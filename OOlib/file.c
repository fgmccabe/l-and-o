...
retCode fileInBytes(ioPo io, byte *ch, long count, long *actual) {
  retCode ret = Ok;
...
retCode fileOutBytes(ioPo io, byte *b, long count, long *actual) {
  retCode ret = Ok;
  long remaining = count;
  filePo f = O_FILE(io);
...
retCode fileSeek(ioPo io, long count) {
  filePo f = O_FILE(io);
