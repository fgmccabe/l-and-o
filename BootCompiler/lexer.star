  ...
  fun nextToken(tkSt,Ops) is let{
    ...
    fun nxTok(St) where hedChar(St) has value hCh is
        switch hCh in {
          ...
          case '\'' is readQuoted(St,'\'',(Str,Lc)=>idTok(Str,Lc))
          case '\"' where hedHedChar(St) has value '\"' and hedHedHedChar(St)='\"' is
            readTripleQuoted(St)
          case '\"' is readString(St)
          case '\`' is readQuoted(St,'\`',(Str,Lc)=>regexpTok(Str,Lc))
    ...
    fun readQuoted(State,Qt,G) is let{
      fun readReg(St,soFar) where hedChar(St) has value Qt is
            report(G(revImplode(soFar),deltaLoc(State,St)),nxSt(St))
    ...
    fun readString(State,Gen) is let{
