...
instruction(depth,26,oLc,nOp,"pick up current stack depth")
instruction(trim,27,iLc,nOp,"trim stack")
instruction(indexi,28,iAh,Ltl,"integer index jump")
instruction(indexs,29,iAh,Ltl,"symbol index jump")
instruction(indexn,30,iAh,Ltl,"numerical index jump")
instruction(indexx,31,iAh,Ltl,"constructor index jump")
instruction(trpblk,32,nOp,nOp,"start error block")
instruction(trpend,33,nOp,nOp,"end error block")
instruction(except,34,iAh,nOp,"raise run-time exception")
instruction(gcmap,35,uAr,lSz,"Set active arguments & local depth")
instruction(gc,36,uAr,lSz,"Invoke GC if not enough space")
instruction(susp,38,iAh,iAm,"suspend/execute call")
instruction(resume,39,oAr,nOp,"continue from suspended call")
instruction(trgr,40,uAr,nOp,"trigger suspended calls")
