...
typedef struct {
  heapPo H;
  /* The heap associated with the copy */
  logical deep;
  varCopyPo vTable;
  /* The table of variable entries */
  long vTop;
...
long spaceNeeded(ptrI src, long *vCount, heapPo H, logical deep) {
  return termSize(src, vCount, H, deep);
}
...
retCode localCopy(ptrPo dst, heapPo H, ptrPo src) {
  ...
  if (reserveSpace(H, (size_t) spaceNeeded(*src, &vCount, H, True)) == Ok) {
    ...
    case objTg: {
      ...
      if (inGlobalHeap(p)) {
        *dst = S;
        ...
        retCode ret = Ok;
        long ix;
        objPo new = allocSpace(&globalHeap, (size_t) objectSize(p));
