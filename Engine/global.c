/*
  A concurrent garbage collector for the L&O engine
...
objPo globalSpace;
/* This is the raw memory block */
static long globalSize;
/* And how big it is */
static cardMap *grey;
/* A mark table for adding grey markers */
static long greySize;
/* Size of the grey table in cardMaps */

HeapRec globalHeap;
/* This is the global heap */

objPo leftBase;
/* The left hand side base */
...
static logical gIsTerm(objPo o) {
  if (isfwd(*(ptrPo) o))
    ...
    return (logical) (oCl->class == classClass);  /* classes have #class as their class */
  }
}
...
static logical gIsSpecial(objPo o) {
  ...
  return (logical) (oCl->class == specialClass); /* special classes have #specialclass as their class */
}
...
void globalGC(long request)    /* how many cells were requested? */
{
  ...
  globalGcRec G =
    {(logical) (globalHeap.base > toHeap.base), &globalHeap, &toHeap};
...
static void scanProcess(processPo p, globalGcPo G) {
  ...
  register long len = envSize(p->proc.cPC);
